...
// hand-written copy of public.system_team_period_summary's `returns table
// (...)` silently fell behind: 20260907150000_p20d added min_calls_target
// to the middle of that column list and this interface was never updated,
// so it had been describing a function shape that no longer existed. Nothing
// here reads that column, so it caused no bug -- but the same stale-copy
// mistake, made against the SQL side, is exactly what made P26's first
// migration fail ("cannot change return type of existing function"). One
// source of truth, regenerated by `npm run types`, cannot drift that way.
//
// The two nullability overrides below are deliberate. Supabase's type
// generator assumes every column a SQL function returns is NOT NULL, which
// is wrong for both of these: pct_calls divides through a nullif() and
// last_logged_at is null for an agent with no activity in the window (a real
// production row returns null for both). Taking the generated types verbatim
// would hand callers a `string` that is sometimes null.
type TeamPeriodSummaryRow = Omit<
  Database['public']['Functions']['system_team_period_summary']['Returns'][number],
  'pct_calls' | 'last_logged_at'
> & {
