-- P19b: leaders (SMDs) also log their own calls/appointments/sales (they
-- have their own Dashboard, Goals, and streak, same as an associate) but
-- private.enqueue_due_notifications() only ever queued evening_nudge/
-- sunday_summary for role = 'associate', so a leader could never receive
-- their own evening nudge or cycle summary regardless of any /settings
-- toggle -- by product decision now widened to include leaders, who keep
-- getting their team cycle digest on top (a leader is the one role that can
-- legitimately receive all three notification kinds). admin is deliberately
-- left out of the widened roles: admin.org_id is always null (P11c) and an
-- admin never logs activity or has a target, so an evening nudge or cycle
-- summary would have nothing to report.
--
-- composeEveningNudge/composeCycleSummary (src/lib/notifications/compose.ts)
-- need no change -- both already key off agent_id/system_effective_target
-- generically, with no role-specific branching.
